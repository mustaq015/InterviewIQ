      ...
      <div className="w-72 border-r overflow-auto transition-all duration-300 flex-shrink-0">
        ...
        <div className="p-3 space-y-2">
          {sortedProjects.map((proj) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <h3 className="font-medium text-sm truncate flex-1">{proj.title}</h3>
                <div className="flex gap-1 flex-shrink-0">
                  <Button variant="ghost" size="icon" className="h-6 w-6" onClick={(e) => { e.stopPropagation(); openEdit(proj); }} title="Edit">
                    <Pencil className="h-3 w-3" />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={(e) => { e.stopPropagation(); handleDelete(proj.id); }} title="Delete">
                    <Trash2 className="h-3 w-3" />
                  </Button>
                </div>
              </div>
              <p className="text-[10px] text-muted-foreground mt-1">{proj.duration}</p>
            </div>
          ))}
